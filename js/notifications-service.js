(function () {
  ...
  const TYPES = new Set(["follow", "comment", "reply", "bookmark"]);
  ...
  async function fetchNotifications(userId, options) {
    ...
    const takeIds = [...new Set(rows.filter((row) => row.type !== "follow" && row.target_id).map((row) => row.target_id))];

    const [profilesResult, takesResult] = await Promise.all([fetchProfilesByIds(actorIds), fetchTakesByIds(takeIds)]);
    ...
    const notifications = rows.map((row) => {
      ...
      const take = row.target_id ? takeMap.get(row.target_id) || null : null;
      const notification = {
        ...row,
        actor,
        take,
