(function () {
  ...
  async function fetchCurrentUserFollowerSet(currentUserId, userIds) {
    if (!currentUserId || !userIds.length) {
      return { followerSet: new Set(), error: null };
    }
    ...
    return {
      followerSet: new Set((result.data || []).map((row) => row.follower_id)),
      error: result.error,
    };
  }
  ...
  window.ClashlyFollows = {
    ...
    followUser,
    unfollowUser,
    fetchFollowingSet: fetchCurrentUserFollowingSet,
    fetchFollowerSet: fetchCurrentUserFollowerSet,
    fetchFollowList,
  };
})();
