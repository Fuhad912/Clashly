(function () {
      ...
      <div id="${DRAWER_ID}" class="comments-drawer" hidden>
        ...
        <section class="comments-drawer__panel" role="dialog" aria-modal="true" aria-labelledby="comments-drawer-title">
          ...
          <header class="comments-drawer__head">
            ...
            <button type="button" class="btn btn--ghost comments-drawer__close" data-close-comments-drawer="true">Close</button>
          </header>
          ...
          <section class="comments-drawer__body">
            <div class="comments-shell">
              <header class="comments-shell__head">
                <div class="comments-shell__title">
                  <p class="comments-shell__eyebrow">Debate floor</p>
                  <h2><span id="comments-drawer-thread-total">0</span> comments</h2>
                  <p>Read the thread first, then jump in with a sharper reply.</p>
                </div>
              ...
              <p id="comments-drawer-state" class="comments-drawer__status feed-state">Loading comments...</p>
          ...
          <div class="comments-drawer__composer">
            <form id="comments-drawer-form" class="comment-compose" novalidate>
              <div class="comment-compose__head">
                <div>
                  <p class="comment-compose__title">Join the debate</p>
                  <p class="comment-compose__subtitle">Reply directly or jump into a side thread.</p>
                </div>
              </div>
              ...
              <footer class="comment-compose__footer">
                <span class="comment-compose__count" id="comments-drawer-count">0/280</span>
                <button type="submit" class="btn btn--primary" id="comments-drawer-submit">Post</button>
              </footer>
            </form>
          </div>
        </section>
  ...
  function updateCountLabel() {
    ...
    if (!input || !countEl || !window.ClashlyComments) return;
    countEl.textContent = `${input.value.length}/${window.ClashlyComments.MAX_COMMENT_LENGTH}`;
  }
  ...
  async function loadComments() {
    ...
    setDrawerState("Loading comments...", "");
    try {
  ...
  function handleShareOpen(input) {
    ...
    window.ClashlyUtils.copyText(input.shareUrl)
      .then(() => setDrawerState("Link copied to clipboard.", "success"))
