(function () {
  ...
  function updateHeader() {
    ...
    if (!currentQuery) {
      document.title = "Clashly | Search";
      if (titleEl) titleEl.textContent = "Discover";
      if (subtitleEl) subtitleEl.textContent = "Explore people, takes, and hashtags moving through Clashly.";
      syncSearchInput();
      return;
    }

    document.title = `Clashly | Search: ${currentQuery}`;
  ...
  function renderTrendingTopics(topics) {
    ...
    if (!topics.length) {
      gridEl.hidden = true;
      gridEl.innerHTML = "";
      setTrendingState("Not enough recent hashtag activity yet.", "");
      return;
    }
  ...
  async function loadTrendingTopics() {
    ...
    setTrendingState("Loading live lanes...", "");
  ...
  function handleShareOpen(input) {
    ...
    window.ClashlyUtils.copyText(input.shareUrl)
      .then(() => setState("Link copied to clipboard.", "success"))
