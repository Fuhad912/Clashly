(function () {
  ...
  async function decorateTakeRows(rows, currentUserId) {
    ...
    const [profileResult, voteResult, bookmarksResult, hashtagsResult] = await Promise.all([
    ...
    return {
      takes: mergeTakesWithProfiles(rowsWithHashtags, profileResult.profiles || []),
      error: null,
    };
  }
  ...
  async function fetchFeedTakes(options) {
    const client = getClientOrThrow();
    const limit = options && options.limit ? options.limit : 35;
  ...
  async function fetchTakesByUser(userId, options) {
    ...
    const rows = takeQuery.data || [];
    return decorateTakeRows(rows, currentUserId);
  }
  ...
  async function fetchBookmarkedTakes(userId, options) {
    ...
    if (!userId) {
      return { takes: [], error: new Error("User ID is required to load saved takes.") };
    }

    const bookmarkQuery = await client
      .from(BOOKMARKS_TABLE)
    ...
    if (bookmarkQuery.error) {
      return { takes: [], error: bookmarkQuery.error };
    }
  ...
  async function fetchTakesByHashtag(tag, options) {
    ...
    const limit = options && options.limit ? options.limit : 40;
    const tab = normalizeTab(options && options.tab ? options.tab : "new");
    const currentUserId = options && options.currentUserId ? options.currentUserId : "";

    if (!safeTag) {
      return { takes: [], error: new Error("Hashtag is required.") };
    }
    ...
    if (hashtagQuery.error) {
      return { takes: [], error: hashtagQuery.error };
    }

    if (!hashtagQuery.data) {
      return { takes: [], error: null, hashtag: safeTag };
    }

    const takeHashtagQuery = await client
      .from(TAKE_HASHTAGS_TABLE)
      .select("take_id")
    ...
    if (takeHashtagQuery.error) {
      return { takes: [], error: takeHashtagQuery.error, hashtag: safeTag };
    }
    ...
    if (takeQuery.error) {
      return { takes: [], error: takeQuery.error, hashtag: safeTag };
    }

    const decoratedResult = await decorateTakeRows(takeQuery.data || [], currentUserId);
    if (decoratedResult.error) {
      return { takes: [], error: decoratedResult.error, hashtag: safeTag };
    }

    return {
      takes: applyTabSorting(decoratedResult.takes || [], tab).slice(0, limit),
      error: null,
      hashtag: safeTag,
    };
  }
  ...
  async function searchTakes(query, options) {
    ...
    return decorateTakeRows(takeQuery.data || [], currentUserId);
  }
